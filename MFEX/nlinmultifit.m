...
% 	end
	...
	for ii = 1:num_curves
		...
		x_vec = [x_vec, reshape(x_cell{ii}, 1, [])]; %#ok<AGROW>
		y_vec = [y_vec, reshape(y_cell{ii}, 1, [])]; %#ok<AGROW>
		mdl_ind2 = mdl_ind2 + length(x_cell{ii});
		mdl_vec = [mdl_vec, sprintf('mdl_cell{%d}(beta,x(%d:%d)), ', ii, mdl_ind1, mdl_ind2)]; %#ok<AGROW>
		
		if found_weights
			if is_wght_func
				wghts_vec = [wghts_vec, sprintf('wghts_cell{%d}(y(%d:%d)), ', ii, mdl_ind1, mdl_ind2)]; %#ok<AGROW>
			else
				wghts_vec = [wghts_vec, reshape(wghts_cell{ii}, 1, [])]; %#ok<AGROW>
			end
		end
